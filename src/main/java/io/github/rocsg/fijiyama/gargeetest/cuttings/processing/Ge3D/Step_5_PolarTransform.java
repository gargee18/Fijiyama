...
public class Step_5_PolarTransform implements PipelineStep{

    public static void main(String[] args)  throws Exception{
        Specimen spec= new Specimen("B_202");
        ImageJ ij=new ImageJ();//Needed for testing
        new  Step_5_PolarTransform().execute(spec,true);
    ...
    public void execute(Specimen specimen,boolean testing) throws Exception {
        ...
        String imgPath =  Config.getPathToHyperstack(specimen);
        String roiPath = Config.getPathToContourROIForGeneralizedPolarTransform(specimen);
        // String roiPath="/mnt/41d6c007-0c9e-41e2-b2eb-8d9c032e9e53/gargee/Cuttings_MRI_registration/B_" + specimen + "/roi_cambium_on_hyperimage/contour.roi";//The path of a PolygonRoi, drawn in the trigonometric direction (counter clockwise)
        ...
        String outputDir = Config.getPathtoPolarTransformsDir(specimen);
        // String outputDir = "/mnt/41d6c007-0c9e-41e2-b2eb-8d9c032e9e53/gargee/Cuttings_MRI_registration/B_"+ specimen + "/Polar_Transform_Results/";
        ...
        for (int j = 0; j < res.length; j++) {
            if (res[j] != null) {  // Ensure the image is valid
                String filePath = outputDir + specimen.getName()+ "_" + names[j].replace(" ", "_") + ".tif"; // Replace spaces with underscores
